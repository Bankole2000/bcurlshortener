export const timeAgo = (datelike) => {
  ...
  const updatedAt = Date.parse(new Date(Number(datelike)));
  ...
  switch (timeScale) {
    ...
    case "day":
      ...
      break;
    case "hour":
      agoInEnglish = `${timeValue} hr${timeValue > 1 ? 's': ''} ago`
      break;
    case "minute":
      agoInEnglish = `${timeValue}m ago`
      break;
    default:
      agoInEnglish = `< 1m ago`
      break;
...
export const formatDate = (dateLike) => {
  const realDate = new Date(Number(dateLike));
